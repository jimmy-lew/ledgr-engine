...
fn main() -> Result<(), LedgerError> {
    ...
    engine.append_transaction(cash_id,    120_000, TransactionType::Credit, "Customer payment #1001")?;
    engine.append_transaction(revenue_id,-120_000, TransactionType::Debit,  "Revenue recognised #1001")?;
    ...
    engine.append_transaction(expense_id,  95_000, TransactionType::Debit,  "Salaries – March")?;
    engine.append_transaction(cash_id,    -95_000, TransactionType::Credit, "Salaries – March")?;

    // Event 4: Founder equity injection $5,000
    engine.append_transaction(cash_id,    500_000, TransactionType::Credit, "Equity injection")?;
    engine.append_transaction(equity_id, -500_000, TransactionType::Debit,  "Equity injection")?;
    ...
    println!("  Debits  (outflows): {:>10} cents ({:.2} USD)",
        summary.total_debits, summary.total_debits as f64 / 100.0);
    println!("  Credits (inflows):  {:>10} cents ({:.2} USD)",
        summary.total_credits, summary.total_credits as f64 / 100.0);
