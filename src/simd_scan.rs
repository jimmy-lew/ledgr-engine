...
//! ## Hardware Dispatch
//!
//! We use Rust's `std::arch` for direct x86_64 AVX2 intrinsics.
//! A **runtime CPU feature check** (`is_x86_feature_detected!`) selects the
//! fast path at startup; the scalar fallback is always present for other
//! architectures (ARM, WASM, etc.) and for test environments.
//!
//! ### Why AVX2 for i64 sums?
//!
//! An AVX2 register holds **256 bits = 4 × i64 lanes**.
//! `_mm256_add_epi64` adds four pairs of 64-bit integers in a single clock
//! cycle, giving a theoretical 4× throughput vs scalar.  In practice with
//! loop pipelining on modern Intel/AMD microarchitectures, the real-world
//! throughput for a streaming sum is closer to **2-3×** due to memory
//! bandwidth being the actual bottleneck for large arrays.
...
///
/// Dispatches to AVX2 SIMD on x86_64 when the CPU supports it, otherwise
/// falls back to a hand-unrolled scalar loop.
...
/// can promote to SIMD, plus the hardware-explicit AVX2 path when available.
pub fn simd_sum_by_type(amounts: &[i64], tx_types: &[u8]) -> (i64, i64) {
    assert_eq!(
        amounts.len(), tx_types.len(),
...
fn scalar_sum_i64(data: &[i64]) -> i64 {
    ...
    for &v in rem { total = total.wrapping_add(v); }
    total
}
...
fn scalar_sum_by_type(amounts: &[i64], tx_types: &[u8]) -> (i64, i64) {
    ...
    for (&a, &t) in amt_rem.iter().zip(type_rem) {
        if t == 0 { debits  = debits.wrapping_add(a); }
        else       { credits = credits.wrapping_add(a); }
    }
...
unsafe fn avx2_sum_by_type(amounts: &[i64], tx_types: &[u8]) -> (i64, i64) {
    ...
    for (a, t) in amt_chunks.zip(type_chunks) {
        ...
        let debit_mask  = _mm256_set_epi64x(
            if t[3] == 0 { -1 } else { 0 },
            if t[2] == 0 { -1 } else { 0 },
            if t[1] == 0 { -1 } else { 0 },
            if t[0] == 0 { -1 } else { 0 },
        );
        let credit_mask = _mm256_set_epi64x(
            if t[3] == 1 { -1 } else { 0 },
            if t[2] == 1 { -1 } else { 0 },
            if t[1] == 1 { -1 } else { 0 },
            if t[0] == 1 { -1 } else { 0 },
...
mod simd_tests {
    ...
    fn simd_split_sum_matches_scalar() {
        let n = 1_000usize;
        let amounts:  Vec<i64> = (0..n).map(|i| if i % 2 == 0 { -(i as i64) } else { i as i64 }).collect();
