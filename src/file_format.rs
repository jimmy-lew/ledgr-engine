...
pub const SEGMENTS_BASE_OFFSET: u64 =
    (FILE_HEADER_SIZE + ACCOUNTS_REGION_SIZE) as u64;            // 131 584
...
impl FileHeader {
    ...
    pub fn read_from<R: Read + Seek>(r: &mut R) -> Result<Self> {
        ...
        if stored_crc != computed {
            return Err(LedgerError::HeaderChecksumMismatch { stored: stored_crc, computed });
        }
...
impl Default for FileHeader { fn default() -> Self { Self::new() } }
...
pub fn write_account_slot<W: Write + Seek>(
    ...
    let offset = FILE_HEADER_SIZE as u64
        + (slot_index * ACCOUNT_RECORD_SIZE) as u64;
...
pub fn read_account_slot<R: Read + Seek>(
    ...
    let kind = AccountType::from_u8(kind_byte)
        .unwrap_or(AccountType::Asset);

    Ok(Some(Account { id, name, kind, created_at, balance }))
}
...
impl SegmentHeader {
    pub fn write_to<W: Write + Seek>(&self, w: &mut W) -> Result<()> {
        ...
        for c in &self.columns { w.write_u64::<LE>(c.offset)?; }
        for c in &self.columns { w.write_u64::<LE>(c.length)?; }
        for c in &self.columns { w.write_u8(c.encoding)?; }
    ...
    pub fn read_from<R: Read>(r: &mut R) -> Result<Self> {
        ...
        for o in offsets.iter_mut()   { *o = r.read_u64::<LE>()?; }
        for l in lengths.iter_mut()   { *l = r.read_u64::<LE>()?; }
        for e in encodings.iter_mut() { *e = r.read_u8()?;        }
        ...
        Ok(Self { magic, row_count, min_ts, max_ts, first_row_global_idx, columns, data_crc32 })
    }
